...
async function clickSuspectedForMember(driver, memberName) {
    ...
    const suspectedBtnXPath =
        `//android.widget.TextView[@text='${memberName}']` +
        `/ancestor::android.view.ViewGroup` +
        `//android.widget.Button[@resource-id='org.piramalswasthya.sakhi.saksham.uat:id/btn_form_tb' and @text='SUSPECTED']`;

    const suspectedBtn = await driver.$(suspectedBtnXPath);
    await suspectedBtn.waitForDisplayed({ timeout: 5000 });
    await suspectedBtn.click();
}
...
async function fillMalariaScreeningDate(driver, day, month, year, fieldHint = 'Treatment Start Date') {
...
async function main() {
    ...
    try {
        ...
        await clickGridItemByText(driver, 'Leprosy Suspected');
        await driver.pause(2000);

        const targetBeneficiary = 'DEBA KARMAKAR';
        await clickSuspectedForMember(driver, targetBeneficiary);
        await driver.pause(2000);

        const isLeprosyConfirmed = "Yes";
        await selectFormRadio(driver, "Has leprosy been confirmed?", isLeprosyConfirmed);
        await driver.pause(1000);

        if (isLeprosyConfirmed === "Yes") {
            await selectFormRadio(driver, "Type of leprosy confirmed", "MB (Multibacillary)");
            await driver.pause(1000);
