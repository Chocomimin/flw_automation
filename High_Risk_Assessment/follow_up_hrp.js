...
async function fillCalendarDate(driver, day, month, year, fieldHint) {
    ...
    const dateFieldXPath = `//android.widget.EditText[contains(@hint, '${fieldHint}')]`;
    ...
    while (currentMonthIndex !== targetMonthIndex && currentMonthIndex !== -1) {
        if (targetMonthIndex > currentMonthIndex) {
            ...
            if (await nextBtn.isExisting() && await nextBtn.isDisplayed()) {
                ...
                await swipeByCoordinates(driver, 700, 1200, 300, 1200);
            }
            currentMonthIndex++;
        } else {
            ...
            if (await prevBtn.isExisting() && await prevBtn.isDisplayed()) {
                ...
                await swipeByCoordinates(driver, 300, 1200, 700, 1200);
            }
            currentMonthIndex--;
        }
...
async function main() {
    ...
    try {
        ...
        console.log("Clicking on 'Follow-up of HRP Women'...");
        await clickGridItemByText(driver, 'Follow-up of HRP Women');
        await driver.pause(1000);
        ...
        console.log(`Clicking FOLLOW UP for: ${targetBeneficiary}...`);
        await clickFollowUpForBeneficiary(driver, targetBeneficiary);
        await driver.pause(2000);

        console.log("Filling out Follow-up Pregnant form...");

        // 1. Date of Visit
        await fillCalendarDate(driver, 31, 3, 2026, 'Date of Visit');
        await driver.pause(1000);

        // 2. High Risk Conditions
        await selectRadioOption(driver, 'RD Malaria', 'No');
        await driver.pause(500);

        await selectRadioOption(driver, 'RD Dengue', 'No');
        await driver.pause(500);

        await selectRadioOption(driver, 'RD Filaria', 'No');
        await driver.pause(500);
        ...
        // 3. Hemoglobin
        await fillTextField(driver, 'Hemoglobin (Hb) Test (g/dl)', '11.5');
        await driver.pause(500);

        // 4. IFA Supplement (Conditional Field)
        const isIFAProvided = 'Yes';
        await selectRadioOption(driver, 'Whether IFA or Folic Acid supplement is provided', isIFAProvided);
        await driver.pause(1000);

        if (isIFAProvided === 'Yes') {
            await fillTextField(driver, 'Issued quantity', '30');
            await driver.pause(500);
        }

        await selectRadioOption(driver, 'Pregnancy Induced Hypertension', 'No');
        await driver.pause(500);
        ...
        await selectRadioOption(driver, 'Gestational Diabetes Mellitus', 'No');
        await driver.pause(500);

        // 7. Blood Glucose Test (Conditional Fields)
        // Set this variable to 'RBS', 'FBS & PPBS', or 'OGTT' to switch logic
        const bloodGlucoseTest = 'OGTT';

        await selectRadioOption(driver, 'Blood Glucose (Sugar) Test', bloodGlucoseTest);
        await driver.pause(1000);

        if (bloodGlucoseTest === 'RBS') {
            await fillTextField(driver, 'Random blood Sugar', '110');
            await driver.pause(500);
        } else if (bloodGlucoseTest === 'FBS & PPBS') {
            await fillTextField(driver, 'Fasting Blood Sugar Test (FBS)', '90');
            await driver.pause(500);
            await fillTextField(driver, 'Post-Prandial Blood Sugar Test', '130');
            await driver.pause(500);
        } else if (bloodGlucoseTest === 'OGTT') {
            await fillTextField(driver, 'Fasting Blood Sugar Test (FBS)', '95');
            await driver.pause(500);
            await fillTextField(driver, 'After 2 Hours', '135');
            await driver.pause(500);
        }

        // 8. Additional Conditions
        await selectRadioOption(driver, 'Hypothyrodism', 'No');
        await driver.pause(500);

        await selectRadioOption(driver, 'Polyhydromnios', 'No');
        await driver.pause(500);

        await selectRadioOption(driver, 'Oligohydromnios', 'No');
        await driver.pause(500);

        // --- NEW FIELDS ---

        // 9. Antepartum Hemorrhage
        await selectRadioOption(driver, 'Antepartum Hemorrhage', 'No');
        await driver.pause(500);

        // 10. Mal Presentation
        await selectRadioOption(driver, 'Mal Presentation', 'No');
        await driver.pause(500);

        // 11. HIV/Syphilis/Hep B
        await selectRadioOption(driver, 'HIV/Syphilis/Hep B', 'No');
        await driver.pause(500);
