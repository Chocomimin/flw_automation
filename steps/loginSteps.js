...
async function selectLanguage(driver, language = "English") {
  const langMap = {
    "English": "rb_eng",
    "Hindi": "rb_hindi",
    "Assamese": "rb_assamese",
    "Bengali": "rb_bangala"
  };

  const resourceId = `org.piramalswasthya.sakhi.saksham.uat:id/${langMap[language]}`;
  const radioBtn = await driver.$(`android=new UiSelector().resourceId("${resourceId}")`);

  await radioBtn.waitForDisplayed({ timeout: 5000 });
  await radioBtn.click();
  console.log(`✅ ${language} language selected`);
  await driver.pause(2000); // Wait for UI transition
}
